...
### hatch rates
hatch_rate <- ovitrap_results_2 %>%
  ungroup() %>%
  mutate(n_unhatched = n_eggs - n_larvae, 
         n_unhatched_2 = if_else(n_unhatched < 0, 0, n_unhatched), 
         perc_unhatched = (n_unhatched_2/n_eggs)*100) %>%
  group_by(cluster, month) %>%
  mutate(avg_unhatched = median(n_unhatched_2, na.rm = T), 
            avg_perc_unhatched = median(perc_unhatched))

viridis_pal <- c(viridis::viridis(n = 3))
months <- c("March" = viridis_pal[1], "April" = viridis_pal[2], "May" = viridis_pal[3])

ggplot(hatch_rate, aes(month, n_unhatched_2))+
  ...
  theme_classic()+
  xlab("Month")+
  ylab("Median No. Unhatched Larvae")+
  theme(axis.text = element_text(size = 16), 
        axis.title = element_text(size = 16, face = "bold"))+
  scale_color_manual(name = "Month", values = months)+
  theme(aspect.ratio = 1)

ggplot(hatch_rate, aes(month, perc_unhatched))+
  geom_violin(aes(fill = month))+
  geom_jitter()+
  #geom_errorbar(aes(ymin = ovitrap_lower_CI, ymax = ovitrap_upper_CI), alpha = 0.15, show.legend = F)+
  #geom_errorbar(aes(xmin = bg_lower_CI, xmax = bg_upper_CI), alpha = 0.15, show.legend = F)+
...
mozzies <- c("Hatched Larvae" = viridis_pal()(4)[1], "Adults" = viridis_pal()(4)[4])
...
#ggsave("~/Documents/Yale EMD/Year 4/Ovitrap AJTMH/figures/fig_2a.pdf", dpi = 1200)
...
#ggsave("~/Documents/Yale EMD/Year 4/Ovitrap AJTMH/figures/fig_2b.pdf", dpi = 1200)
...
#ggsave("~/Documents/Yale EMD/Year 4/Ovitrap AJTMH/figures/fig_2c.pdf", dpi = 1200)
...
#ggsave("~/Documents/Yale EMD/Year 4/Ovitrap AJTMH/figures/fig_3.pdf", dpi = 1200)
...
ggplot(pred_mod_df_3, aes(n_wmel_bg, mod5_count))+
  ...
  theme(axis.text = element_text(size = 16), 
        axis.title = element_text(size = 18, face = "bold"))+
  theme(aspect.ratio = 1) 
#ggsave("~/Documents/Yale EMD/Year 4/Ovitrap AJTMH/figures/fig_4a.pdf", dpi = 1200)
...
ggplot(pred_mod_df_3, aes(n_wmel_bg, mod2_count))+
  ...
  theme(axis.text = element_text(size = 16), 
        axis.title = element_text(size = 18, face = "bold"))+
  theme(aspect.ratio = 1) 
#ggsave("~/Documents/Yale EMD/Year 4/Ovitrap AJTMH/figures/fig_4c.pdf", dpi = 1200)
...
ggplot(pred_mod_df_3, aes(bg, mod5_prev))+
  ...
  theme(axis.text = element_text(size = 16), 
        axis.title = element_text(size = 18, face = "bold"))+
  theme(aspect.ratio = 1)
#ggsave("~/Documents/Yale EMD/Year 4/Ovitrap AJTMH/figures/fig_4b.pdf", dpi = 1200)
...
ggplot(pred_mod_df_3, aes(bg, mod2_prev))+
  ...
  theme(axis.text = element_text(size = 16), 
        axis.title = element_text(size = 18, face = "bold"))+
  theme(aspect.ratio = 1)
#ggsave("~/Documents/Yale EMD/Year 4/Ovitrap AJTMH/figures/fig_4d.pdf", dpi = 1200)
...
weighted_model <- lm(trap_prop ~ cluster_prev, 
                     data = ovi_clust_trap, 
                     weights = n_larvae)

slope <- coef(weighted_model)[["cluster_prev"]]
intercept <- coef(weighted_model)[["(Intercept)"]]
r2 <- summary(weighted_model)$r.squared

## plot
ovi_clust_trap %>%
  ggplot(aes(x = cluster_prev, y = trap_prop)) +
  geom_point(aes(size = n_larvae, color = month)) +
  geom_smooth(aes(weight = n_larvae), method = "lm", color = "black", 
              se = FALSE, linewidth = 1) +
  labs(x = "Cluster-level wMel Prevalence", 
       y = "Trap-level wMel Prevalence", 
       color = "Month", 
...
weighted_model_bg <- lm(trap_prop ~ cluster_prev, 
                     data = bg_clust_trap, 
                     weights = total_aegypti_caught)

slope_bg <- coef(weighted_model_bg)[["cluster_prev"]] # 0.9795015
intercept_bg <- coef(weighted_model_bg)[["(Intercept)"]]
r2_bg <- summary(weighted_model_bg)$r.squared # 0.661008


### prevalence
bg_clust_trap %>%
  ggplot(aes(x = cluster_prev, y = trap_prop))+
  geom_point(aes(size = total_aegypti_caught, color = month))+
  geom_smooth(aes(weight = total_aegypti_caught), method = "lm", color = "black", 
              se = FALSE, linewidth = 1) +
  labs(x = "Cluster-level wMel Prevalence", 
